...
export function PersonalBest({ activities, onSelectActivity }: PersonalBestProps) {
  ...
  return (
    <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl px-4 py-3 transition-all duration-300 hover:shadow-lg hover:shadow-[var(--color-accent)]/5 hover:border-[var(--color-accent)]/30 hover:bg-[var(--color-accent)]/5">
      ...
      <div className="divide-y divide-[var(--color-border)]">
        {bests.map(({ key, activity, time }) => (
          ...
          >
            <span className="text-xs text-[var(--color-text)]">{labels[key]}</span>
            <span className={`text-xs font-mono font-bold ${activity ? 'text-[var(--color-accent)]' : 'text-[var(--color-muted)]'}`}>
